...
public class RangeProofVerifier<T extends GroupElement<T>> implements Verifier<GeneratorParams<T>, T, RangeProof<T>> {
	...
	@Override
	public void verify(GeneratorParams<T> params, T input, RangeProof<T> proof, Optional<BigInteger> salt)
		...
		T P = a.add(s.multiply(x)).add(gs.sum().multiply(z.negate())).add(hPrimes.commit(hExp))
				.subtract(base.h.multiply(mu)).add(u.multiply(t));
